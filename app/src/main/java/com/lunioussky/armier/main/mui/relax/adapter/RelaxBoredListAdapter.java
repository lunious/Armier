...
public class RelaxBoredListAdapter extends BaseQuickAdapter<RelaxBoredListBean, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, RelaxBoredListBean item) {
        helper.setText(R.id.bored_author, item.getComment_author());
        helper.setText(R.id.bored_date, DateUtil.getTimestampString(DateUtil.string2Date(item.getComment_date(), "yyyy-MM-dd HH:mm:ss")));
        helper.setText(R.id.bored_content, item.getText_content());
        ...
        GlideApp.with(mContext)
                ...
                .fitCenter()
                .into((ImageView) helper.getView(R.id.bored_item));
    }
}
